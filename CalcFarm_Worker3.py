...
def send_to_work_server(worker_id, work_server_ip, route_url, data, input_list=None):
    """
    ...
    input_list = [worker_id] + input_list
    return post_to_route(route_url, data, work_server_ip , input_list=input_list)


def connect_to_work_server(worker_id, work_server_ip, route_url, input_list=None):
    """
...
def get_file(worker_id, work_server_ip, py_name):
    """
    ...
    file_data = connect_to_work_server(worker_id, work_server_ip, "getpyfile", input_list=[py_name])
...
@Repeat
def python_executor(worker_id, work_server_ip,  script_name, args=None):
    """
    ...
    if not os.path.isfile(file_dir):
        get_file(worker_id, work_server_ip, script_name)
    try:
...
def get_work_unit(worker_id, work_server_ip):
    """
    ...
    """
    work_unit = connect_to_work_server(worker_id, work_server_ip, "getworkunit")
    return work_unit


def update(worker_id, work_server_ip, results, calc_status=WorkUnitCalculationStatusNames.calculated.value):
    """
    ...
    :param calc_status: The status of the calculation process of the work unit.
    """
    ...
    send_to_work_server(worker_id, work_server_ip, 'update', data=worker_log)
...
def task_calc():
    worker_id = None
    try:
        task_py_name, work_server_ip, worker_id = worker_sign_up()
        results = []
        work_status = WorkerStatusNames.working.value
        start = time.perf_counter()
        while work_status != WorkerStatusNames.shut_down.value:
            print('Started working...')
            work_unit = get_work_unit(worker_id, work_server_ip)
            if 'fail_message' in work_unit:
                if work_unit['fail_message'] == WorkServerStatusNames.finished_work.value:
                    work_status = WorkerStatusNames.shut_down.value
                elif work_unit['fail_message'] == WorkServerStatusNames.no_work.value:
                    time.sleep(7)
            else:
                ...
                py_results = None
                try:
                    py_results = python_executor(worker_id, work_server_ip, task_py_name, [first_num, last_num])
                    print('Calculated work unit successfully')
                    update(worker_id, work_server_ip, py_results)
                except WorkUnitError as e:
                    print(e)
                    print("Fix the error in this worker. "
                          "In the meanwhile, some other worker will handle the work unit")

                    update(worker_id, work_server_ip, None, WorkUnitCalculationStatusNames.failed.value)
                    time.sleep(1)
        ...
        print("Computer no' {} finished its work! in {} seconds!".format(str(worker_id), round(end-start, 2)))
        ...
        if worker_id:
            connect_to_work_server(worker_id, work_server_ip, "quit")
